...
# Supercronic for the Laravel scheduler.
RUN arch="$(uname -m)" \
    && case "$arch" in \
...
###########################################
# Build frontend assets with Node/npm
###########################################
...
# Build tooling (vite, plugins, wayfinder) lives in devDependencies, so force
# their install even though NODE_ENV=production would otherwise omit them.
...
HEALTHCHECK --start-period=5s --interval=10s --timeout=5s --retries=8 CMD healthcheck || exit 1
