...
<template>
    ...
    <div class="flex flex-col gap-6 p-4 md:p-6">
        ...
        <div class="rounded-2xl border bg-card p-4 shadow-sm">
            <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-6">
                ...
                <div class="relative lg:col-span-2">
                    ...
                    <input v-model="form.q" :placeholder="$t('jobs.filters.search') + '…'" class="w-full rounded-xl border bg-background py-2.5 pl-9 pr-3 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500/40" />
                </div>
                ...
                <select v-model="form.category" class="rounded-xl border bg-background px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500/40">
                    <option value="">{{ $t('common.all') }} — {{ $t('jobs.filters.category') }}</option>
                    <option v-for="c in categories" :key="c" :value="c">{{ c }}</option>
                </select>

                <!-- Skill (datalist suggestions, free text) -->
                <input v-model="form.skill" list="skill-options" :placeholder="$t('jobs.filters.skill')" class="rounded-xl border bg-background px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500/40" />
                ...
                <select v-model="form.state" class="rounded-xl border bg-background px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500/40">
                    <option value="">{{ $t('common.all') }} — {{ $t('jobs.filters.state') }}</option>
                ...
                <select v-model="form.city" :disabled="!form.state" class="rounded-xl border bg-background px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500/40 disabled:opacity-50">
                    <option value="">{{ form.state ? $t('common.all') + ' — ' + $t('jobs.filters.city') : $t('jobForm.selectStateFirst') }}</option>
        ...
        <div v-if="jobs.data.length" class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            >
                ...
                <p class="mt-1.5 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground">
                    <span class="inline-flex items-center gap-1"><CalendarDays class="size-3.5" /> {{ $t('jobs.posted') }} {{ fmtDate(job.created_at) }}</span>
                    <span v-if="job.expires_at" class="inline-flex items-center gap-1 font-medium text-rose-500 dark:text-rose-400">{{ $t('jobs.applyBy') }} {{ fmtDate(job.expires_at) }}</span>
                </p>
        ...
        <div v-else class="rounded-2xl border border-dashed bg-card px-5 py-16 text-center">
            ...
            <p class="mt-4 font-medium">{{ $t('jobs.noJobs') }}</p>
