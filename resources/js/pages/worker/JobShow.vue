        ...
        <PageHeader :icon="Briefcase" :title="job.title" :description="employerLine">
            <template #action>
                <button
                    ...
                    @click="toggleSave"
                >
                    <Bookmark class="size-4" :fill="isSaved ? 'currentColor' : 'none'" /> {{ isSaved ? $t('jobs.savedJob') : $t('jobs.saveJob') }}
                </button>
            </template>
        </PageHeader>
        ...
        <div class="grid gap-6 lg:grid-cols-3">
                ...
                <div class="rounded-2xl border bg-card p-6 shadow-sm">
                    ...
                    <div class="mt-5 grid gap-3 sm:grid-cols-2">
                        <div class="flex items-center gap-3 rounded-xl border p-4">
                            ...
                            <div><div class="text-xs text-muted-foreground">{{ $t('jobs.wage') }}</div><div class="font-semibold">{{ wage }}</div></div>
                        </div>
                        <div class="flex items-center gap-3 rounded-xl border p-4">
                            ...
                            <div><div class="text-xs text-muted-foreground">{{ $t('jobs.vacancies') }}</div><div class="font-semibold">{{ job.vacancies }}</div></div>
                        </div>
                    </div>
                    ...
                    <div v-if="job.perks?.length" class="mt-5 border-t pt-5">
                        <h2 class="mb-2 flex items-center gap-1.5 text-sm font-semibold text-orange-600 dark:text-orange-300"><Gift class="size-4" /> {{ $t('jobs.perks') }}</h2>
                    ...
                    <div class="mt-5 border-t pt-5">
                        <h2 class="mb-2 text-sm font-semibold text-orange-600 dark:text-orange-300">{{ $t('jobs.description') }}</h2>
            ...
            <div>
                <div class="sticky top-6 rounded-2xl border bg-card p-6 shadow-sm">
                    ...
                    <!-- Apply / call -->
                    <div v-else>
                        <h2 class="text-base font-bold">{{ $t('jobs.interested') }}</h2>
                        ...
                        <form v-else-if="canApply" class="mt-4 space-y-3" @submit.prevent="submitApply">
                            <div>
                                <label class="mb-1.5 block text-xs font-medium text-muted-foreground">{{ $t('jobs.coverNote') }}</label>
                                <textarea v-model="form.cover_note" rows="4" :placeholder="$t('jobs.coverNotePlaceholder')" class="w-full rounded-xl border bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500/40"></textarea>
                                <p v-if="form.errors.cover_note" class="mt-1 text-xs text-rose-500">{{ form.errors.cover_note }}</p>
                            </div>
                            <div>
                                <label class="mb-1.5 block text-xs font-medium text-muted-foreground">{{ $t('jobs.expectedWage') }}</label>
