...
const inputClass =
    'w-full rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2 text-sm text-white placeholder:text-slate-500 outline-none transition focus:border-teal-400/50 focus:bg-white/[0.06]';
const selectClass = inputClass + ' [&>option]:bg-[#0a1a15]';
</script>
...
<template>
    ...
    <div class="dark relative min-h-screen overflow-hidden bg-[#04100d] text-slate-200 antialiased">
        <div class="pointer-events-none absolute inset-0 -z-10">
            <div class="absolute left-1/2 top-[-10%] h-[420px] w-[680px] -translate-x-1/2 rounded-full bg-teal-600/20 blur-[140px]"></div>
        ...
        <main class="mx-auto max-w-5xl px-5 py-10">
            ...
            <div class="mb-8 rounded-2xl border border-white/10 bg-white/[0.03] p-4 backdrop-blur">
                <div class="mb-3 flex items-center gap-2 text-sm font-semibold text-teal-300">
                    <SlidersHorizontal class="size-4" /> {{ t('common.filter') }}
                </div>
                <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-6">
                    <div class="lg:col-span-2">
                        <div class="relative">
                            <Search class="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-slate-500" :class="searching && 'animate-pulse text-teal-400'" />
            ...
            <div v-if="jobs.data.length" class="grid gap-4 sm:grid-cols-2">
                <Link
                    ...
                    :key="job.id"
                    :href="`/jobs/${job.id}`"
                    class="group relative overflow-hidden rounded-2xl border border-white/10 bg-white/[0.03] p-5 transition duration-300 hover:-translate-y-1 hover:border-teal-400/40 hover:bg-white/[0.05]"
                >
            ...
            <div v-if="jobs.links.length > 3" class="mt-8 flex flex-wrap justify-center gap-1">
                <Link
                    ...
                    :class="[
                        ...
                        link.active ? 'bg-gradient-to-r from-teal-500 to-cyan-600 text-white' : 'text-slate-400 hover:bg-white/5',
